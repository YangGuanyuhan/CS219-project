...
VECTOR_DOT="../src/vector_dot_product"
...
if [ ! -f "$VECTOR_DOT" ]; then
    echo "Error: vector_dot_product not found at $VECTOR_DOT"
    echo "Building vector_dot_product..."
    (cd ../src && make vector_dot_product)
    if [ ! -f "$VECTOR_DOT" ]; then
        echo "Failed to build vector_dot_product"
        exit 1
    fi
fi
...
# 运行测试并提取时间
echo "Running vector_dot_product with generated test cases..."
